<?php
    ...
    function loadOwner(){//función para cargar propietarios
        //Invocamos una clase para realizar consultas 
        $queries = new consultas();
        //Genera consulta en la tabla owner para obtener los propietarios registrados
        $result=$queries->showOwners();

        if (!isset($result)) {//En caso de haya un error en la variable resultado
            modalAlert('No hay propietarios registrados, ¡registra uno!' , '../../view/owners/registerOwner.php' , 'warning' , '3');
        }else{//Si no se encuentra un error se realizara la maquetación de la tabla para visualizar los propietarios
            ...
            foreach ($result as $f){
                echo    '<tr>
                            ...
                            <td class="colum5 colum6-users">
                                <a class="icon-table"><i class="fas fa-info-circle info-title" data-title="Información completa" onclick="viewOwner(this)" id="'.$f["document"].'"></i></a>      
                            </td> 
                            <td class="colum5 colum6-users  d-flex"> 
                                <a class="icon-table"><i class="fas fa-trash info-title" data-title="Eliminar propietario" onclick="deleteOwner(this)" id="'.$f["document"].'"></i></a>
                            </td>
                        </tr>';
            }//end foreach
            echo '</tbody>';
        };//end if
    };// ./ cierre función para cargar propietarios
    function ownerInformation($id){
        //Invocamos una clase para realizar consultas del propietario 
        $queries = new consultas();
        //Genera consulta en la tabla owner
        $result=$queries->ownerInfo($id);

        if (!isset($result)) {
            echo '<h2> No hay propietario para mostrar</h2>';
        } else {
            foreach ($result as $f) {
                echo    '<div class="row">
